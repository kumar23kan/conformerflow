...
def compute_ramachandran_quality(torsions: np.ndarray) -> dict:
    """
    Check pseudo-torsion angles fall in reasonable ranges.
    torsions: (N, L-3) pseudo-torsion angles in radians

    Returns fraction of angles in allowed regions.
    """
    if torsions.size == 0:
        return {"rama_allowed_fraction": 1.0}

    # Allowed region: pseudo-torsions should be broadly distributed
    # Real Ramachandran uses phi/psi from N/CA/C atoms
    # Here we use a simple range check on pseudo-torsions
    in_range = (torsions >= -np.pi) & (torsions <= np.pi)
    allowed  = float(in_range.mean())
    return {"rama_allowed_fraction": allowed}
...
def compute_clash_score(ca_coords: np.ndarray,
    ...
    for n in range(N):
        ...
        # Mask bonded pairs and diagonal
        ...
        clashes = (dmat[mask] < clash_threshold).sum()
        total_clashes += clashes
        total_pairs   += mask.sum()

    return {
        "clash_score":    float(total_clashes / (N * L)),
