...
def _build_query(nodes: list, start: int, rows: int) -> dict:
    """Assemble a well-formed RCSB v2 search payload."""
    return {
        "query": {
            "type":             "group",
...
def _execute_search(query: dict, retries: int = 3) -> list:
    """
    POST one page of results to the RCSB v2 search endpoint.
    Retries up to `retries` times with exponential back-off on transient errors.
    Returns list of PDB-ID strings (empty list on persistent failure).
    """
    for attempt in range(retries):
        try:
            ...
            logger.warning(f"RCSB search error (attempt {attempt+1}/{retries}): {e}")
    ...
    logger.error("RCSB search failed after all retries.")
    return []


def _paginated_search(nodes: list, max_results: int) -> list:
    """
    Fetch up to max_results IDs by issuing multiple paginated requests
    (RCSB v2 caps each response at RCSB_PAGE_SIZE = 10 000 rows).
    """
    all_ids: list = []
    start         = 0

    while len(all_ids) < max_results:
        rows  = min(RCSB_PAGE_SIZE, max_results - len(all_ids))
        query = _build_query(nodes, start=start, rows=rows)
        page  = _execute_search(query)

        if not page:
            break                      # exhausted or error

        all_ids.extend(page)
        start += len(page)

        if len(page) < rows:
            break                      # last page returned fewer rows → done

    logger.info(f"Search returned {len(all_ids)} entries (requested ≤ {max_results}).")
    return all_ids
...
def fetch_deposition_dates(pdb_ids: list, max_workers: int = 8) -> dict:
    """
    Batch-fetch deposition dates from RCSB REST API.
    Returns dict mapping pdb_id (uppercase) -> "YYYY-MM-DD" or None.
    """
    def _fetch_one(pdb_id):
        url = f"{RCSB_DATA_URL}/{pdb_id.upper()}"
        try:
            resp = requests.get(url, timeout=15)
            resp.raise_for_status()
            data = resp.json()
            # deposit_date format: "YYYY-MM-DDTHH:MM:SS+00:00" or "YYYY-MM-DD"
            date_str = (data.get("rcsb_accession_info", {})
                            .get("deposit_date", ""))
            if date_str:
                return pdb_id.upper(), date_str[:10]
        except Exception:
            pass
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(_fetch_one, pid): pid for pid in pdb_ids}
        for future in tqdm(as_completed(futures), total=len(pdb_ids),
                           desc="Fetching deposition dates"):
            pid, date = future.result()
            results[pid] = date

    n_found = sum(1 for v in results.values() if v is not None)
    logger.info(f"Fetched deposition dates: {n_found}/{len(pdb_ids)} found")
    return results
