...
def relative_frame_encoding(R: torch.Tensor,
    ...
    # R has ROWS as frame axes (build_backbone_frames convention).
    # Correct invariant relative frame for this convention:
    #   R_rel = R_i @ R_j^T        (invariant under global SE(3))
    #   t_rel = R_i @ (t_j - t_i)  (invariant under global SE(3))
    # See: for global rotation G, new R_i = R_i @ G^T, so
    #   new R_i @ (new R_j)^T = (R_i @ G^T) @ (R_j @ G^T)^T = R_i @ G^T @ G @ R_j^T = R_i @ R_j^T ✓
    R_T = R.transpose(-1, -2)   # R_j^T when expanded over j

    # Pairwise expansion
    R_i_exp   = R.unsqueeze(-3).expand(*shape[:-3], L, L, 3, 3)   # R_i
    R_j_T_exp = R_T.unsqueeze(-4).expand(*shape[:-3], L, L, 3, 3) # R_j^T
    t_i_exp   = t.unsqueeze(-2).expand(*t.shape[:-2], L, L, 3)
    t_j_exp   = t.unsqueeze(-3).expand(*t.shape[:-2], L, L, 3)

    R_rel      = torch.matmul(R_i_exp, R_j_T_exp)          # R_i @ R_j^T
    R_rel_flat = R_rel.reshape(*shape[:-3], L, L, 9)

    dt    = (t_j_exp - t_i_exp).unsqueeze(-1)               # (..., L, L, 3, 1)
    t_rel = torch.matmul(R_i_exp, dt).squeeze(-1)           # R_i @ (t_j - t_i)
